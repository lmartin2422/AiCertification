...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # 1. Mark the cell as a move that has been made and safe
        self.moves_made.add(cell)
        self.mark_safe(cell)

        # 2. Determine neighbors that are not already marked as safe or mines
        neighbors = set()
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
        ...
        # 3. Add a new sentence based on the surrounding cells
        if neighbors:
            self.knowledge.append(Sentence(neighbors, count))

        # 4. Iterate over the knowledge base and mark additional mines or safes
        changes = True
        while changes:
            changes = False
            safes = set()
            ...
            # Look at each sentence to see if it has new safe cells or mines
            for sentence in self.knowledge:
        ...
        # 5. Deduce new sentences from existing knowledge
        new_sentences = []
